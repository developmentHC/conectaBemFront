...
function formatDateTime(detail?: IAppointmentDetail) {
  const iso =
    detail?.dateTime ??
    (detail?.date && detail?.time ? `${detail.date}T${detail.time}` : detail?.date);
...
export default function CancelAppointmentPage() {
    ...
    return (
      <main className="mx-auto max-w-3xl px-4 py-10">
        <section className="rounded-lg border border-slate-200 bg-white p-8 text-center">
          ...
          <Link
            href="/agendamentos-pacientes"
            className="mt-4 inline-flex items-center gap-2 font-semibold text-blue-700 text-sm"
          >
    ...
    return (
      <main className="mx-auto max-w-3xl px-4 py-10">
        <div className="rounded-lg border border-slate-200 bg-white p-8 text-center">
          ...
          <p className="mt-3 text-slate-500 text-sm">Carregando informações do agendamento...</p>
        </div>
      </main>
    );
  }
  ...
  const professionalImage =
    data.professional?.profileImageUrl ??
    data.professional?.image ??
    "/images/professional/woman (1).jpeg";
    ...
    <div className="min-h-screen bg-[#f3f5fa] px-3 py-10 sm:px-2">
      <main className="mx-auto w-full max-w-5xl space-y-6">
        <Link
          ...
          className="inline-flex items-center gap-2 font-semibold text-[#3857F4] text-sm"
        >
          <ArrowLeft className="h-4 w-4" />
          Voltar
        </Link>
        <div>
          <p className="font-semibold text-2xl text-[#1F2340]">Cancelar agendamento</p>
          <p className="font-medium text-[#8B90B3] text-xs">ID Serviço: {serviceIdLabel}</p>
        </div>

        <section className="space-y-6 rounded-lg border border-[#E1E4FF] bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <span
              className={`inline-flex w-full items-center justify-center gap-2 rounded-xl border px-4 py-2 font-semibold text-sm ${statusTokens.border} ${statusTokens.bg} ${statusTokens.text}`}
            >
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
            <div className="] relative h-28 w-28 overflow-hidden rounded-lg border border-[#E0E4FF] bg-white">
              <Image
                ...
                alt={data.professional?.name ?? "Profissional"}
                fill
                className="object-cover"
                sizes="112px"
              />
            </div>
            <div className="space-y-2 text-[#1F2340] text-sm">
              <h2 className="font-semibold text-[#2843A4] text-xl">
          ...
          <hr className="mx-auto my-1 w-[98%] border-[#CBC8D0] border-t" />
        ...
        <section>
          <div>
            <p className="font-semibold text-[#1F2340] text-sm">Serviços a serem cancelados</p>
            <p className="text-[#8B90B3] text-xs">
              Todos os serviços que serão cancelados com este(a) profissional.
            </p>
              ...
              {services.length ? (
                services.map((service) => {
                  ...
                  return (
                    <article
                      key={service.id}
                      className="rounded-lg border border-[#E0E4FF] bg-[#FDFDFE] p-4"
                    >
                      <p className="font-semibold text-[#1F2340] text-base">{service.name}</p>
                      <div className="mt-3 grid gap-3 text-[#4B5278] text-sm sm:grid-cols-2">
                        <p>
                          <span className="font-semibold text-[#1F2340]">Duração:</span>{" "}
                          {durationLabel}
                        </p>
                        <p>
                          <span className="font-semibold text-[#1F2340]">Preço:</span>{" "}
                          {servicePrice}
                        </p>
                      </div>
                      <p className="mt-3 text-[#6E749D] text-sm">{serviceDescription}</p>
                    </article>
                  );
                })
              ) : (
                <p className="text-[#8A8FB8] text-sm">
                  Nenhum serviço cadastrado para cancelamento.
                </p>
              )}
            </div>
          </div>
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            <div>
              ...
              <textarea
                ...
                className="mt-2 w-full resize-none overflow-hidden rounded-lg border border-[#E0E4FF] bg-[#F8F9FF] px-4 py-3 text-[#1F2340] text-sm focus:border-[#3857F4] focus:bg-white focus:outline-none"
              />
              <div className="mt-2 flex justify-end text-[#8B90B3] text-xs">
                <span>
                  {String(reasonCount).padStart(3, "0")}/{MAX_REASON_LENGTH}
                </span>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex flex-col gap-3">
                <div className="flex items-start gap-3 rounded-lg border border-[#ADBAEB] bg-[#FFF3F3] px-4 py-3 text-[#1D1B20] text-sm">
                  <Info className="mt-0.5 h-4 w-4 text-[#C53030]" />
                  <span>
                    Atenção! Esse cancelamento não poderá ser desfeito ao clicar no botão abaixo.
                  </span>
                </div>
                <button
                  type="submit"
                  disabled={isSubmitting}
                  className="inline-flex items-center justify-center gap-3 self-center rounded-lg border-2 border-[#C53030] bg-white px-28 py-2 font-semibold text-[#C53030] text-base transition-colors hover:bg-[#FFF3F3] disabled:cursor-not-allowed disabled:opacity-70"
                >
                  {isSubmitting ? (
                    <>
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/20 px-4 py-8 backdrop-blur-sm">
          <div className="relative w-full max-w-lg rounded-lg bg-white p-6 shadow-xl">
            <button
              ...
              className="absolute top-4 right-4 rounded-full p-1 text-[#667085] transition hover:bg-slate-100"
            >
              <X className="h-4 w-4" />
            </button>
            <div className="space-y-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="font-semibold text-[#1F2340] text-lg">Confirmar cancelamento?</p>
                  <p className="text-[#4B5278] text-sm">
                    Essa ação notificará o profissional e liberará o horário selecionado.
                  </p>
                </div>
              </div>
              <div className="rounded-lg border border-[#FFE1E1] bg-[#FFF5F5] px-4 py-3 text-[#6E1F1F] text-sm">
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
                <button
                  type="button"
                  onClick={() => setIsConfirmModalOpen(false)}
                  className="inline-flex items-center justify-center rounded-lg border border-[#E0E4FF] px-5 py-2 font-semibold text-[#4B5278] text-sm hover:bg-[#F8F9FF]"
                >
                  Manter agendamento
                </button>
                <button
                  ...
                  disabled={isSubmitting}
                  className="inline-flex items-center justify-center gap-2 rounded-lg border-2 border-[#C53030] bg-[#C53030] px-6 py-2 font-semibold text-sm text-white transition hover:bg-[#A11F1F] disabled:cursor-not-allowed disabled:opacity-70"
                >
                  {isSubmitting ? (
                    <>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20 px-4 py-8 backdrop-blur-sm">
          <div className="relative w-full max-w-xl rounded-lg bg-white p-8 shadow-2xl">
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                <p className="font-semibold text-[#1F2340] text-lg">Cancelamento efetuado</p>
                <p className="text-[#4B5278] text-sm">Seu agendamento foi cancelado com sucesso!</p>
              </div>
            </div>
            <div className="mt-6 flex w-full justify-center">
              <button
                type="button"
                onClick={closeSuccessModal}
                className="inline-flex min-w-[200px] items-center justify-center rounded-lg bg-[#3857F4] px-6 py-3 font-semibold text-sm text-white shadow-[#3857F4]/25 shadow-lg"
              >
                Voltar para Meus Agendamentos
              </button>
            </div>
