...
describe('Fluxo de Cadastro', () => {
  ...
  context('Validações do campo de e-mail', () => {
    it('Impede continuar com campo de e-mail vazio', () => {
      ...
      cy.get('.gap-7 > .MuiButtonBase-root')
        .should('be.disabled');
    });
  ...
  context('Cadastro completo com código via e-mail (Mail.tm)', () => {
    ...
    it.skip('Realiza cadastro com sucesso usando OTP recebido por e-mail', () => {
      ...
      getLatestEmail(token).then((message) => {
        return getEmailContent(token, message.id);
      }).then((emailRes) => {
        const emailBody =
          emailRes.body.text || emailRes.body.html || '';
      ...
      cy.get('.go2072408551')
        .should('have.text', 'Cadastro realizado e sessão iniciada!');
    });
  });
});