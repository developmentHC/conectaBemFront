  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-10 text-center">
      ...
      <Link
        ...
        className="mt-6 inline-flex items-center gap-2 font-semibold text-blue-700 text-sm"
      >
...
export default function AppointmentDetailPage() {
  ...
  const services =
    data.services ??
    (data.serviceName ? [{ id: data.serviceId ?? "service", name: data.serviceName }] : []);
  ...
  const observationCount =
    observationText === "Nenhuma observação registrada." ? 0 : observationText.length;
      ...
      <div className="mx-auto w-full max-w-6xl space-y-6">
        ...
        <section className="rounded-lg p-6">
          <header className="space-y-1">
            <h1 className="font-semibold text-2xl text-[#1F2340]">Informações do Agendamento</h1>
            <p className="font-medium text-[#8B90B3] text-xs">ID Serviço: {serviceIdLabel}</p>
          </header>

          {normalizedStatus === "pending" ? (
            <div className="mt-5 flex flex-col gap-3">
              <div className="flex flex-col gap-3 rounded-lg border border-[#C53030] bg-[#FFFFFF] p-4 text-[#1D1B20] text-sm">
                <div className="flex items-start gap-3">
                  <AlertTriangle className="h-5 w-5 flex-shrink-0 text-[#C53030]" />
                  <p>
                    A partir de agora o profissional terá que confirmar a sua solicitação. Você será
                    notificado assim que o agendamento for aceito.
                  </p>
                </div>
              </div>
            </div>
          ...
          <article className="mt-6 rounded-lg border border-[#E1E4FF] bg-[#FFF] p-6">
            ...
            <div className="mt-6 space-y-5">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                ...
                <div>
                  ...
                  <div className="mt-4 space-y-2 text-[#1F2340] text-sm">
                    <div className="flex items-center gap-3">
                      <CalendarDays className="] h-4 w-4 text-[#1d1d20" />
                      <p className="font-medium text-base">
                        <span className="] font-semibold text-[#1d1d20">Data:</span> {date}
                      </p>
                    </div>
                    <div className="flex items-center gap-3">
                      <Clock3 className="] h-4 w-4 text-[#1d1d20" />
                      <p className="font-medium text-base">
                        <span className="] font-semibold text-[#1d1d20">Horário:</span> {time}
                      </p>
                    </div>
                    <div className="flex items-center gap-3">
                      <Wallet className="] h-4 w-4 text-[#1d1d20" />
                      <p className="font-medium text-base">
                        <span className="] font-semibold text-[#1d1d20">Preço:</span>{" "}
                        {formattedPrice}
                      </p>
                    </div>
                  </div>
              ...
              <hr className="mx-auto w-[98%] border-[#CBC8D0] border-t" />
              <div className="flex items-start gap-3 px-4">
                <MapPin className="h-5 w-5 text-[#9790A2]" />
                <p className="font-semibold text-[#1F2340] text-sm">{formattedAddress}</p>
              </div>
              <hr className="mx-auto w-[98%] border-[#CBC8D0] border-t" />
            </div>

            <div className="mt-6 px-4 py-3">
              <p className="font-semibold text-[#1F2340] text-sm">Serviços Agendados:</p>
              {services.length ? (
                <ul className="mt-3 list-disc space-y-1 pl-5 text-[#4B5278] text-sm">
                  {services.map((service: AppointmentService) => (
                    <li key={service.id}>{service.name}</li>
                  ))}
                </ul>
              ) : (
                <p className="mt-3 text-[#8A8FB8] text-sm">Nenhum serviço cadastrado.</p>
              )}
            </div>
            ...
            <div className="mt-6 flex flex-col gap-3 md:flex-row">
              <Link
                href={contactMailto}
                className="inline-flex w-full items-center justify-center rounded-lg bg-[#3857F4] px-6 py-3 text-center font-semibold text-sm text-white shadow-[#3857F4]/30 shadow-lg md:flex-1"
              >
              ...
              {canCancel ? (
                <Link
                  ...
                  className="inline-flex w-full items-center justify-center rounded-lg border border-[#C53030] bg-white px-6 py-3 font-semibold text-[#C53030] text-sm md:flex-1"
                >
          ...
          <div className="mt-6 flex items-start gap-3 rounded-lg border border-[#F3E1E1] bg-[#FFF3F3] px-4 py-3 text-[#1D1B20] text-sm">
          ...
          <section className="mt-6 space-y-6">
            <div className="rounded-lg border border-[#E1E4FF] bg-white/60 p-5">
              <div className="flex items-center justify-between">
                <p className="font-semibold text-[#1F2340] text-sm">Suas observações</p>
                <button
                  type="button"
                  disabled
                  className="font-semibold text-[#9BA1C7] text-xs opacity-70"
                >
                  editar
                </button>
              </div>
              <div className="mt-3 min-h-[120px] rounded-lg border border-[#E0E4FF] bg-[#F8F9FF] p-4 text-[#4B5278] text-sm">
                {observationText}
                <div className="mt-3 text-right text-[#A1A5C8] text-xs">
                  {String(observationCount).padStart(3, "0")}/300
                </div>
              </div>
            </div>

            <div className="rounded-lg p-5">
              <p className="font-semibold text-[#1F2340] text-sm">Endereço Selecionado</p>
              <p className="text-[#8B90B3] text-xs">
                Veja no mapa o endereço escolhido para realizar o seu atendimento.
              </p>
              ...
              <div className="mt-4 overflow-hidden rounded-lg border border-[#E0E4FF] bg-[#E3E9FF]">
                {mapImageSrc ? (
                  <Image
                    src={mapImageSrc}
                    alt="Mapa do endereço do atendimento"
                    width={1024}
                    height={400}
                  ...
                  <div className="flex h-56 w-full flex-col items-center justify-center bg-[radial-gradient(circle_at_top,_#dfe6ff,_#b8c7ff)] text-[#31428B]">
                    ...
                    <p className="mt-2 font-semibold text-sm">Prévia do mapa indisponível</p>
                  </div>
                )}
                {mapsLinkUrl ? (
                  <Link
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="block bg-[#3857F4] px-6 py-3 text-center font-semibold text-sm text-white"
                  >
                    Abrir no mapa
                  </Link>
                ) : null}
